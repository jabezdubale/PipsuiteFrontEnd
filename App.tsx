...
import { getTrades, saveTrade, deleteTrades, getAccounts, saveAccount, getTagGroups, saveTagGroups, getStrategies, saveStrategies, saveTrades } from './services/storageService';
...
import { X, ChevronDown, Calculator, TrendingUp, TrendingDown, RefreshCw, ExternalLink, Loader2, Upload, Plus, Trash2, Clipboard, ChevronUp, Eraser } from 'lucide-react';
...
function App() {
      ...
      for (let i = 0; i < items.length; i++) {
        if (items[i].type.indexOf("image") !== -1) {
          ...
          if (blob) {
            ...
            // We don't prevent default here to allow normal paste if needed, unless we processed it
          }
        }
      }
    };
  ...
  const handleAnalyzeImage = async (base64: string) => {
    setIsAnalyzing(true);
    try {
        // We do NOT add the image to screenshots here, as requested.
        // It is only used for analysis.
  ...
  // Executes deletion logic (Soft Delete or Permanent Delete based on context)
  const executeDelete = async () => {
      ...
      try {
          ...
          if (isPermanentDelete) {
              ...
              // Balance was already reversed during soft delete, so we just remove the data.
              ...
              tradesToTrash.forEach(t => {
                  if (t.isBalanceUpdated && t.accountId && t.pnl !== 0) {
                      // We are deleting, so we reverse the PnL (New Balance = Old - PnL)
                      // Accumulate total PnL to subtract
              ...
              const updatedTrades = trades.map(t => {
                  if (tradesToDelete.includes(t.id)) {
                      return { 
                          ...
                          // We preserve isBalanceUpdated to check on restore if it needs to be re-applied
                      };
                  }
                  return t;
              });
              
              // Save updated state to storage (simulating batch update via saveTrade one by one or better add a bulk update method, 
              // for now saveTrade updates existing ID so we loop)
              // Ideally storageService should have a `updateTrades` method. We'll use local state and one save for simplicity or loop.
              // Simulating bulk save by saving all modified.
  ...
  const handleRestoreTrades = async (ids: string[]) => {
      try {
          ...
          // 1. Re-apply Balance Impact
          tradesToRestore.forEach(t => {
              // Only apply balance change if the trade was originally marked to update balance
              if (t.isBalanceUpdated && t.pnl !== 0 && t.accountId) {
                  // Re-apply PnL (New Balance = Old + PnL)
                  balanceAdjustments[t.accountId] = (balanceAdjustments[t.accountId] || 0) + t.pnl;
              }
          });
          ...
          // 2. Update Trades (Remove Deleted Flag)
          const updatedTrades = trades.map(t => {
              if (ids.includes(t.id)) {
                  return { 
                      ...
                      isDeleted: false, 
                      deletedAt: undefined,
                      // Preserve original isBalanceUpdated value
                  };
              }
              return t;
          });
  ...
  // Image Handlers for Add Modal (Manual Upload)
  ...
  // Navigates to View Modal instead of Edit Page
  const navigateToTrade = (trade: Trade) => {
    setSelectedTradeId(trade.id);
    // Don't clear selectedDailyDate so the underlying modal stays open
    setIsViewModalOpen(true); 
  };
  
  // Transition from View Modal to Edit Page
  ...
  const handleQuickAdd = (e: React.FormEvent) => {
    ...
    const newTrade: Trade = {
      ...
      type: determinedType,
      createdAt: isoString, // Open Time (Log Time)
      entryDate: isoString, // Primary date bucket
      entryTime: undefined, // Empty initially
      entrySession: Session.NONE, // No session because no time yet
      ...
      setup: 'SMC', // Default strategy set to SMC
    ...
    if (subView === 'detail' && selectedTradeId) {
       ...
       if (trade) {
         return <TradeDetail 
            ...
            onDelete={(id) => handleRequestDelete([id])} // Use new delete handler
            onBack={() => setSubView('list')} 
            accounts={accounts} // Passing accounts here
            tagGroups={tagGroups}
            strategies={strategies}
            onUpdateBalance={handleUpdateBalance} // Passing balance updater
         />;
       }
    }
      ...
      case 'journal':
        return (
          <TradeList 
            ...
            onDeleteTrade={(id) => handleRequestDelete([id])} // Use new delete handler
            onDeleteTrades={handleRequestDelete} // Use new delete handler for bulk
            onImportTrades={handleImportTrades}
            tagGroups={tagGroups} // Pass tagGroups
          />
        );
      case 'trash':
        return (
          <TradeList 
            trades={trashTrades}
            selectedAccountId={selectedAccountId}
            onTradeClick={() => {}} // Disabled in trash
            onDeleteTrade={(id) => handleRequestDelete([id])} // Permanent delete
                 ...
                 {accounts.map(acc => (
                   <li key={acc.id} className="flex justify-between items-center p-3 bg-background rounded border border-border hover:border-primary/50 transition-colors">
                     <div className="flex flex-col">
                        ...
                        <span className="text-xs text-textMuted">{acc.isDemo ? 'Demo Account' : 'Live Account'} - {acc.currency}</span>
                     </div>
    ...
    >
      ...
      {isViewModalOpen && selectedTradeForView && (
          <TradeViewModal 
              ...
              onDelete={() => handleRequestDelete([selectedTradeForView.id])} // Use new handler
              onSave={handleSaveTrade}
              tagGroups={tagGroups}
              onUpdateBalance={handleUpdateBalance}
          />
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto">
              <form id="add-trade-form" onSubmit={handleQuickAdd} className="p-5 pt-4 space-y-4">
                  ...
                  {/* Lot Size & Risk % - Conditional Styling */}
                  <div className="grid grid-cols-2 gap-4">
                    <div>
            ...
            <div className="p-4 border-t border-border flex gap-3 shrink-0 bg-surface rounded-b-xl">
                  ...
                  <button 
                      type="submit" 
                      form="add-trade-form"
                      className="flex-1 bg-primary hover:bg-blue-600 text-white py-2 rounded-lg font-bold text-sm shadow-md"
                  >
                      Save Trade
                  </button>
            </div>
