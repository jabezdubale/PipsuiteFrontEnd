    ...
    <div className="bg-surface border border-border rounded-xl p-4 shadow-sm mt-4">
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        {strategies.map((strategy, index) => {
          ...
          if (isEditing) {
             return (
                 <div key={index} className="flex items-center gap-1 bg-surfaceHighlight rounded px-1.5 py-0.5 border border-primary">
                     <input 
                         autoFocus
                         value={editValue}
                         onChange={(e) => setEditValue(e.target.value)}
                         onKeyDown={(e) => e.key === 'Enter' && saveEdit()}
                         className="w-20 bg-transparent text-xs outline-none text-textMain"
                     />
                     <button onClick={saveEdit} className="text-profit hover:bg-profit/10 rounded p-0.5"><Check size={10}/></button>
                     <button onClick={() => setEditingIndex(null)} className="text-loss hover:bg-loss/10 rounded p-0.5"><XCircle size={10}/></button>
                 </div>
             )
          }
          ...
          return (
            <div key={index} className="group flex items-center gap-1.5 px-2 py-1 bg-surface border border-border rounded-md text-xs text-textMain hover:border-primary/50 transition-all">
              ...
              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity pl-1 border-l border-border/50 ml-1">
                ...
                <button onClick={() => handleDelete(index)} className="text-loss hover:text-red-600"><X size={10}/></button>
              </div>
            </div>
          );
        })}
