...
import { Plus, X, Edit2, Check, XCircle, ChevronDown, ChevronUp } from 'lucide-react';
                            ...
                            <div className="mt-3 pt-3 border-t border-border animate-in fade-in slide-in-from-top-1">
                                <div className="flex flex-wrap gap-1.5 mb-3">
                                    {group.tags.map((tag, tagIdx) => {
                                        ...
                                        if (isEditing) {
                                            return (
                                                <div key={tagIdx} className="flex items-center gap-1 bg-surfaceHighlight rounded px-1 py-0.5 border border-primary">
                                                    <input 
                                                        autoFocus
                                                        value={editingTag.value}
                                                        onChange={(e) => setEditingTag({...editingTag, value: e.target.value})}
                                                        onKeyDown={(e) => e.key === 'Enter' && saveEdit()}
                                                        className="w-16 bg-transparent text-[10px] outline-none text-textMain"
                                                    />
                                                    <button onClick={saveEdit} className="text-profit hover:bg-profit/10 rounded p-0.5"><Check size={10}/></button>
                                                    <button onClick={() => setEditingTag(null)} className="text-loss hover:bg-loss/10 rounded p-0.5"><XCircle size={10}/></button>
                                                </div>
                                            )
                                        }
                                        ...
                                        return (
                                            <div key={tagIdx} className="group flex items-center gap-1 px-1.5 py-0.5 bg-surface border border-border rounded-md text-[10px] text-textMuted hover:border-primary/50 transition-all">
                                                ...
                                                <div className="flex gap-0.5 ml-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    ...
                                                    <button onClick={() => handleDeleteTag(groupIdx, tagIdx)} className="text-loss hover:text-red-600 p-0.5"><X size={8}/></button>
                                                </div>
                                            </div>
                                        )
                                    })}
